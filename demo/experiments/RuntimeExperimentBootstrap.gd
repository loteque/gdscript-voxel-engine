...
func _ready() -> void:
	...
	add_child(_ui)
	_ui.configure(TITLE, "Initializing runtime workload experiment…", MODE_LABELS, MODE_DESCRIPTIONS)
	...
	_ui.set_ready("Startup 1/3: UI ready. Loading streaming fixture manifest…")
	await get_tree().process_frame

	_manifest = ResourceLoader.load(MANIFEST_PATH) as TerrainChunkManifest
	_ui.set_ready("Startup 2/3: Manifest loaded. Loading experiment matrix…")
	await get_tree().process_frame

	_matrix = ResourceLoader.load(MATRIX_PATH) as StreamingExperimentMatrix
	_ui.set_ready("Startup 3/3: Validating experiment configuration…")
	await get_tree().process_frame
