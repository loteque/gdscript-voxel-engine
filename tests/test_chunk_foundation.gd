...
func _test_adjacent_chunk_boundary_samples_match() -> void:
	...
	var boundary_a := Vector3i(chunk_a.point_field.cell_dimensions.x, 2, 2)
	var boundary_b := Vector3i(0, 2, 2)
	var sampling_position_a := chunk_a.point_field.get_sampling_position(boundary_a)
	var sampling_position_b := chunk_b.point_field.get_sampling_position(boundary_b)
	...
	_assert_float_equal(
		chunk_a.point_field.get_density(boundary_a),
		chunk_b.point_field.get_density(boundary_b),
