...
func _run() -> void:
	...
	_assert_true(streamer != null, "Residency validation scene must contain ChunkStreamer.")
	_assert_true(target != null, "Residency validation scene must contain an explicit Node3D target.")
	_assert_true(status_label != null, "Residency validation UI must expose runtime status separately from ChunkStreamer.")
	_assert_true(pause_button != null and reset_button != null, "Residency validation scene must expose its target controls.")

	if streamer == null or target == null:
		_finish(scene)
		return

	_assert_true(streamer.manifest != null, "Residency validation scene must load its baked TerrainChunkManifest.")
	...
	# Pause automatic motion so the remainder of the validation is deterministic.
	...
	_assert_coordinates_equal(
		...
		initial_expected,
		"Initial validation residency must contain the available 3 x 1 x 3 neighborhood."
	)
	_assert_equal(streamer.get_loaded_coordinates().size(), 9, "Initial validation residency must load nine available baked chunks.")
	...
	# Move across a chunk boundary and invoke the same public residency operation
	# that automatic target tracking uses. This must load the new edge and unload
	# the obsolete edge without regenerating or meshing anything at runtime.
	...
	_assert_true(not streamer.is_chunk_loaded(Vector3i(-2, 0, 0)), "Chunks outside the moved residency region must unload.")
	_assert_true(streamer.is_chunk_loaded(Vector3i(2, 0, 0)), "Newly desired baked chunks must become resident.")
	...
	_assert_coordinates_equal(
		streamer.get_loaded_coordinates(),
		moved_expected,
		"Duplicate residency updates must remain idempotent in the validation scene."
	)
