...
## space. Boundary faces require a layer of cells on both sides of a crossed
## sample edge, so chunk-region callers must provide suitable overlap/halo data.


# [b]Constants[/b]
# Defines the twelve cell edges using PointFieldResource corner ordering.
...
# [b]Public API[/b]
# Converts an authoritative scalar field into a triangle mesh.

## Generates a new mesh for [param field] at [param iso_level].
##
## Only current field data is accepted. Structurally complete but stale channels
## remain useful to visualizers, but must not be treated as meshing truth.
func generate_mesh(field: PointFieldResource, iso_level: float = 0.0) -> ArrayMesh:
	var mesh := ArrayMesh.new()
	if field == null:
		push_error("SurfaceNetsMesher requires a PointFieldResource.")
		return mesh
	if not field.is_data_current():
		push_error("SurfaceNetsMesher requires current point-field data.")
		return mesh
	...
	_generate_cell_vertices(field, iso_level, vertices, cell_vertex_indices)
	if vertices.is_empty():
		return mesh

	var indices := PackedInt32Array()
	_generate_surface_indices(field, iso_level, cell_vertex_indices, indices)
	if indices.is_empty():
		return mesh
	...
	arrays[Mesh.ARRAY_NORMAL] = _generate_vertex_normals(vertices, indices)
...
# [b]Cell Vertices[/b]
# Places one averaged edge-intersection vertex in every active field cell.
		...
		for y in field.cell_dimensions.y:
			for x in field.cell_dimensions.x:
				...
				var surface_vertex: Variant = _calculate_cell_surface_vertex(
					field, cell_coordinates, iso_level
				)
...
func _calculate_cell_surface_vertex(
	...
	for edge in CELL_EDGES:
		...
		intersection_sum += _interpolate_edge_intersection(
			positions[corner_a], positions[corner_b], density_a, density_b, iso_level
		)
...
# Stitches active cell vertices around sign-changing lattice edges.
	...
	for z in range(1, field.cell_dimensions.z):
		for y in range(1, field.cell_dimensions.y):
			for x in field.cell_dimensions.x:
				...
				if _edge_crosses_iso_surface(density_a, density_b, iso_level):
					_append_edge_quad(
						field, cell_vertex_indices, indices,
						[
							Vector3i(x, y - 1, z - 1), Vector3i(x, y, z - 1),
							Vector3i(x, y, z), Vector3i(x, y - 1, z),
						],
						density_a >= iso_level
					)
			...
			for y in field.cell_dimensions.y:
				...
				if _edge_crosses_iso_surface(density_a, density_b, iso_level):
					_append_edge_quad(
						field, cell_vertex_indices, indices,
						[
							Vector3i(x - 1, y, z - 1), Vector3i(x - 1, y, z),
							Vector3i(x, y, z), Vector3i(x, y, z - 1),
						],
						density_a >= iso_level
					)
			...
			for z in field.cell_dimensions.z:
				...
				if _edge_crosses_iso_surface(density_a, density_b, iso_level):
					_append_edge_quad(
						field, cell_vertex_indices, indices,
						[
							Vector3i(x - 1, y - 1, z), Vector3i(x, y - 1, z),
							Vector3i(x, y, z), Vector3i(x - 1, y, z),
						],
						density_a >= iso_level
					)
...
func _append_edge_quad(
	...
	if reverse_winding:
		indices.append_array(PackedInt32Array([
			quad[0], quad[3], quad[2], quad[0], quad[2], quad[1],
		]))
	else:
		indices.append_array(PackedInt32Array([
...
# [b]Normals[/b]
# Builds smooth area-weighted normals matching Godot clockwise front faces.
...
# [b]Interpolation[/b]
# Finds where an iso-surface crosses a sampled field edge.

func _edge_crosses_iso_surface(density_a: float, density_b: float, iso_level: float) -> bool:
...
func _interpolate_edge_intersection(
	...
	var weight := clampf((iso_level - density_a) / density_delta, 0.0, 1.0)
